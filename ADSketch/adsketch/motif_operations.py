...
def online_anomaly_detection(adaptive_learning, m, p, max_anomalous_cluster_size,
    ...
    if not os.path.exists(offline_pattern_dir):
        ...
        # Note: This call generates the pattern file if it doesn't exist.
        # Its return value is not needed here as it's primarily for pattern generation.
    ...
    # This block seems to re-evaluate the offline results for logging/plotting,
    # but the online detection is based on the loaded patterns.
    if os.path.exists(offline_pattern_dir):
        _, scaled_test_metrics = scale_two_metrics(train_metric_values, test_metric_values)
        # The result of this evaluation is not captured, which is fine for this block.
    ...
    # Handle cases where online_subseq_num might be less than 1 (no subsequences to predict)
    if online_subseq_num <= 0:
        logging.warning(f"Not enough data points for online detection. Online subsequences num: {online_subseq_num}. Returning default evaluation scores.")
        return [0., 0., 0.] # Or [1., 1., 1.] if no anomalies are expected and nothing can be predicted
    ...
    if max_anomalous_cluster_size is None:
        if len(anomalous_clusters) == 0:
            # If no anomalous clusters from offline, determine a min size.
            # Be careful if cluster_sizes is also empty here.
            if len(cluster_sizes) > 0:
                max_anomalous_cluster_size = np.min(np.array(cluster_sizes))
            else:
                max_anomalous_cluster_size = 1 # Default to 1 if no clusters exist
        else:
    ...
    if adaptive_learning:
        ...
        if len(anomalous_clusters) == 0:
            # Handle case where anomalous_clusters is empty, benign_max_dist should still be calculated if cluster_radii exists
            anomalous_max_dist = 0.0 # No anomalous clusters, so max_dist is 0
            if len(cluster_radii) > 0:
                benign_max_dist = np.max(cluster_radii)
            else:
                benign_max_dist = 0.0 # No clusters at all
        else:
            cluster_radii_tmp = np.array(cluster_radii)
            anomalous_max_dist = np.max(cluster_radii_tmp[anomalous_clusters])
            # Create a mask for benign clusters to get their max radius
            benign_indices = [i for i in range(len(cluster_radii)) if i not in anomalous_clusters]
            if len(benign_indices) > 0:
                benign_max_dist = np.max(cluster_radii_tmp[benign_indices])
            else:
                benign_max_dist = 0.0 # No benign clusters
        ...
        logging.info(f'max_anomalous_cluster_size: {max_anomalous_cluster_size}, '
                     f'max_benign_cluster_size: {np.max(cluster_sizes) if len(cluster_sizes) > 0 else 0}') # Add check for empty cluster_sizes
        ...
        logging.info(f'anomalous cluster num: {len(anomalous_clusters)}, '
                     f'benign cluster num: {len(cluster_radii) - len(anomalous_clusters)}')
        # Add check for empty cluster_sizes before np.max
        max_anom_size = np.max(np.array(cluster_sizes)[anomalous_clusters]) if len(anomalous_clusters) > 0 and len(cluster_sizes) > 0 else 0
        max_benign_size = np.max(cluster_sizes) if len(cluster_sizes) > 0 else 0
        logging.info(f'max_anomalous_cluster_size: {max_anom_size}, '
                     f'max_benign_cluster_size: {max_benign_size}')
    ...
    # Capture the result of evaluate_predictions and return it
    final_evaluation_result = evaluate_predictions(m, online_anomalous_subseqs, online_scaled_test_metrics,
                                                   online_test_metric_labels, online_fig)
    return final_evaluation_result # Add this return statement!