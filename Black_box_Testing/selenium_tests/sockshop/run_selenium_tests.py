...
class ChromeEdgeTestRunner:
    ...
    def setup_environment(self):
        ...
        try:
            ...
            if response.status_code == 200:
                print(f"✅ SockShop服务正常 ({self.base_url})")
            else:
                print(f"⚠️ SockShop服务状态: {response.status_code}")
        except Exception as e:
            print(f"❌ SockShop连接失败: {e}")
            return False
    ...
    def run_browser_test(self, browser):
        ...
        # 生成报告文件名
        report_file = os.path.join(self.reports_dir, f"{browser}_complete_report.html")
        ...
        try:
            # 执行测试，实时显示输出
            result = subprocess.run(cmd, timeout=1200)  # 20分钟超时
    ...
    def run_all_browser_tests(self):
        """执行所有浏览器测试"""
        self.print_header("Chrome + Edge 双浏览器完整测试")
...
def main():
    """主函数"""
    runner = ChromeEdgeTestRunner()
    
    runner.print_header("Chrome + Edge 双浏览器测试执行器")
